...
if [[ $HOSTNAME == summit* ]]; then
    ...
    SKIP_TESTS="MPI_TEST_testphdf5_cchunk3|MPI_TEST_testphdf5_tldsc|MPI_TEST_testphdf5_ecdsetw|MPI_TEST_t_shapesame|MPI_TEST_t_filters_parallel"

    perl -i -pe "s/^ctest.*/ctest . -E MPI_TEST_ -C Release -j 32 -T test >& ctestS.out/" hdf5-$HDF5_VER/bin/batch/ctestS.lsf.in.cmake
    perl -i -pe "s/^ctest.*/ctest . -R MPI_TEST_ -E 'MPI_TEST_testphdf5_cchunk3|MPI_TEST_testphdf5_tldsc|MPI_TEST_testphdf5_ecdsetw|MPI_TEST_t_shapesame|MPI_TEST_t_filters_parallel' -C Release -T test >& ctestP.out/" hdf5-$HDF5_VER/bin/batch/ctestP.lsf.in.cmake
