...
sub select_server_and_directory {
   ...
   my $serverfile;
   my $directorynum;
# Only one type of directory for NPOESS at present
#   if ($tes ne "true") {
#      $tes = "";
#      $serverfile = "/home/lrknox/perlscripts/EOSdirs.list";
#      $directorynum = $doy % 76;
#   } else {
      $serverfile = "/home/lrknox/perlscripts/NPOESSdirs.list";
      $directorynum = $doy % 26;
#   }
   my $lineno = 0;
...
#using this one at least initially for NPOESS files
sub download_files {
   my $day = shift;
   my $filecount = 0;
   foreach (@{$jpdir}) {
      @ls = split;
      #print "\nPrinting all 8 fields:\n";
      #foreach (@ls) {
      #   print $_, "\n";
      #}
      # skip hidden, parent and self!
      ...
      next unless ($ls[8] =~ /\.he?5$/); 
      print "Match:  ".$this."/".$ls[8],"\n";
      $success = $Net::FTP::ftpobj->get($ls[8], "/mnt/scr1/NPOESS/selected/$ls[8]");
      if ($success eq "") {
...
my $tes = "";
my $servers = 0;
# ONLY one server for NPOESS at this point
#while ($servers < 2) {
   ...
   $Net::FTP::ftpobj -> binary or
           die "Cannot set mode to binary: $!\n";
...
# No subdirs for NPOESS at present
   push @dirstack,$sdir;
#   if ($tes ne "true") {
