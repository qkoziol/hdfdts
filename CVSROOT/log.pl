...
$cvsroot = $ENV{'CVSROOT'};
$host_type = $ENV{'HOSTTYPE'};
$host_os = `uname -s | tr -d "\012" `;	# a hack.  *sigh*
...
$mailcmd = "| $mail_prog -s 'CVS update: $srepos'";
...
##if($srepos=~m/hdf5/) {
##    use Cwd; # Used for running scripts in the current directory
##
##    ($cwd)=(cwd()=~m#(.*?hdf5)/.*#);
###print STDERR $cwd,"\n";
##    $cwd .= "/bin/versinc";
##    system $cwd;
###   system $cwd == 0 or die "system $cwd failed: $?";
##}
