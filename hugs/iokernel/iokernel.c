...
int main(int argc, char **argv) {
	...
	while (fgets(linebuf,MAXLINE,simFP)) {
		...
		if (row != col || (row != 64 && row != 128 && row != 256 && row != 512 && row != 1024 && row != 2048 && row != 4096 && row != 8192)) {
	...
	*/
    ...
    if (!strcmp(vfd,"d")){
        ...
        if ( (H5Pset_alignment(file_access_properties, (hsize_t)1024, (hsize_t)4*1024)) < 0){
            printf("Failed to align data on file.");
            exit (-1);
        }
	...
	/* Start measuring post time */	  
	gettimeofday(&start_time,NULL);
	
	for (i=0; i<numStreams; i++) {
		rawData_xfer[i] = frame_count[i]*(long)meta_main[i].rows*(long)meta_main[i].cols*sizeof(short);
	...
	printf("Pre-time=%8.3f\n",((float)pre_wtime)/1000000);
	printf("Post-time=%8.3f\n",((float)post_wtime)/1000000);
	
	/* Obtain the time and speed for writing data for each stream */
	for(i=0;i<numStreams;i++) {
		...
		printf("stream %d raw data size =%8.3f MB\n",i,(float)rawData_xfer[i]*MBpB);
		printf("stream %d RawWriteSpeed =%8.3f MB/s\n",i,(float)rawData_xfer[i]*MBpB/(((float)raw_stream_wtime[i])/1000000));
		printf("stream %d meta data size =%8.3f MB\n",i,(float)metadata_xfer[i]*MBpB);
		printf("stream %d MetaWriteSpeed =%8.3f MB/s\n",i,(float)metadata_xfer[i]*MBpB/(((float)meta_stream_wtime[i])/1000000));
		
		total_rawData_xfer += rawData_xfer[i];
		total_xfer += rawData_xfer[i] + metadata_xfer[i];
		total_wtime += total_stream_wtime[i];
		
	}
	
	printf("RawTotalSpeed =%8.3f MB/s\n",(float)total_rawData_xfer*MBpB/((((float)(total_wtime+pre_wtime+post_wtime))/1000000)));
		
	printf("Total size of transferred data =%8.3f MB\n",(float)total_xfer*MBpB);
	printf("TotalSpeed =%8.3f MB/s\n",(float)total_xfer*MBpB/(((float)(total_wtime+pre_wtime+post_wtime))/1000000));
